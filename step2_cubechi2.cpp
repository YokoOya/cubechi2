...
public:
    ...
    char filename[lenfilename], card[lenline], cunit[3][lenline], *token;
    fitsfile *fptr;
    double *valData;
    ...
    bool skipCell(double *ip, bool f_threshold, double threshold4skip, int weight_sign);
...
void Cubedata::readFits(char* filename_in) {
    ...
    for (int i = 0; i < sizeof(cunit) / sizeof(cunit[0]); ++i) {
        if (i == 0) {
            if (fits_read_card(fptr, "CUNIT1", card, &status)) printerror(status);
        } else if (i == 1) {
            if (fits_read_card(fptr, "CUNIT2", card, &status)) printerror(status);
        } else if (i == 2) {
            if (fits_read_card(fptr, "CUNIT3", card, &status)) printerror(status);
        }
        token = strtok(card, "'");
        token = strtok(NULL, "'");
        sprintf(cunit[i], "%s", token);
    }
    ...
    for (int i = 0; i < naxis; ++i) {
        ...
        if (i == 2) {
            if (strstr(cunit[i], "km")) {
                crval[i] *= 1.0e3;        // km/s -> m/s
                cdelt[i] *= 1.0e3;
            }
        } else {
            if (!strstr(cunit[i], "deg")) {
                printf("WARN :: The cunit[%d] is not 'deg' but '%s'.  If the unit is different between the data to be compared, the comparison will be failed.\n", i + 1, cunit[i]);
            }
        }
    }
...
bool Cubedata::skipCell(double *ip, bool f_threshold = false, double threshold4skip = 0., int weight_sign = 1) {
    double val = noval;
    for (int i = 0; i < naxis; ++i) {
    ...
    if (f_threshold) {
        getVal(ip, &val);
        //if (threshold4skip > 0.04 && val > 0) printf("ip = [%3.2le, %3.2le, %3.2le] -> val = %le, threshold = %le\n", ip[0], ip[1], ip[2], val, threshold4skip);
        val *= weight_sign;
...
public:
    ...
    bool skipCell_andThreshold(double *ip, double threshold4skip_Cube1_in, double threshold4skip_Cube2regrid_in, int weight_sign_Cube2);
    bool skipCell_orThreshold(double *ip, double threshold4skip_Cube1_in, double threshold4skip_Cube2regrid_in, int weight_sign_Cube2);
    bool skipCell_region4comparing(double *ip, bool f_threshold, double threshold4skip_Cube1_in, double threshold4skip_Cube2regrid_in, int weight_sign_Cube2);
    void regrid();
...
bool Cubepair::skipCell_andThreshold(double *ip, double threshold4skip_Cube1_in, double threshold4skip_Cube2regrid_in, int weight_sign_Cube2 = 1) {
    return skipCell(ip) || (Cube1->skipCell(ip, true, threshold4skip_Cube1_in) && Cube2regrid->skipCell(ip, true, threshold4skip_Cube2regrid_in, weight_sign_Cube2));
}

bool Cubepair::skipCell_orThreshold(double *ip, double threshold4skip_Cube1_in, double threshold4skip_Cube2regrid_in, int weight_sign_Cube2 = 1) {
    return skipCell(ip) || (Cube1->skipCell(ip, true, threshold4skip_Cube1_in) || Cube2regrid->skipCell(ip, true, threshold4skip_Cube2regrid_in, weight_sign_Cube2));
}

bool Cubepair::skipCell_region4comparing(double *ip, bool f_threshold = false, double threshold4skip_Cube1_in = 0., double threshold4skip_Cube2regrid_in = 0., int weight_sign_Cube2 = 1) {
    for (int i = 0; i < naxis; ++i) {
        if (ip[i] < rIP[i][0] || ip[i] > rIP[i][1]) return true;
    }
    
    if (f_threshold) {
        return skipCell_orThreshold(ip, threshold4skip_Cube1_in, threshold4skip_Cube2regrid_in, weight_sign_Cube2);
    } else {
        return skipCell(ip);
    }
}
...
void Cubepair::setRegion(double *limP_in) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        for (int iaxis = 0; iaxis < naxis; ++iaxis) {
            if (ip_dum[iaxis] < 0) ip_dum[iaxis] = 0;
            if (ip_dum[iaxis] >= Cube1->npix[iaxis]) ip_dum[iaxis] = Cube1->npix[iaxis] - 1;
    ...
    sprintf(str_region, "");
    for (int iaxis = 0; iaxis < naxis; ++iaxis) {
        sprintf(str_dum, "\taxis %d [0, %ld] (pix) <-> [%.8f, %.8f]", iaxis, Cube1->npix[iaxis] - 1, p_dum[iaxis], p_dum2[iaxis]);
        if (iaxis == naxis - 1) {
            strcat(str_dum, " (m/s), ");
        } else {
            strcat(str_dum, " (deg), ");
    ...
    sprintf(str_region, "");
    for (int iaxis = 0; iaxis < naxis; ++iaxis) {
        sprintf(str_dum, "\taxis %d [%d, %d] (pix) <-> [%.8f, %.8f]", iaxis, rIP[iaxis][0], rIP[iaxis][1], rP[iaxis][0], rP[iaxis][1]);
        if (iaxis == naxis - 1) {
            strcat(str_dum, " (m/s), ");
        } else {
            strcat(str_dum, " (deg), ");
...
void Cubepair::calcRatio() {
    ...
    for (int ipix = 0; ipix < Cube1->npixels; ++ipix) {
        ...
        if (skipCell_orThreshold(ip, threshold_Cube1, threshold_Cube1 / weight4Cube2regrid, int(weight4Cube2regrid / abs(weight4Cube2regrid)))) continue;
    ...
    if (abs(weight4Cube2regrid) < EPS) {
        printf("\n\tERROR :: The normalization constant is 0. \n");
        printf("\t\t\tweight = %le\n\n", weight4Cube2regrid);
        exit(1);
    }
...
void Cubepair::setRatio() {
    ...
    printf("Maximum intensity is: \tCube1 -> %.4e\n", maxval_Cube1);
    printf("                    : \tCube2 -> %.4e (after regridding)\n", maxval_Cube2regrid);
    printf("\t-> The initial value for the normalization constant: %.4e\n\n", weight4Cube2regrid);
